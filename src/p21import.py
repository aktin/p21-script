...
class CSVReader(ABC):
  ...
  def save_df_as_csv(self, df_input: pd.DataFrame, path_output: str,
      encoding: str):
    df_input.to_csv(
        path_output, sep=self.CSV_SEPARATOR, encoding=encoding, index=False
    )
...
class CSVPreprocessor(CSVReader, ABC):
  ...
  def _write_header_to_csv(self, header: str):
    ...
    encoding = self.get_csv_encoding(self.PATH_CSV)
    with open(self.PATH_CSV, "r+", encoding=encoding) as f1, open(
        path_dummy, "w+", encoding=encoding
    ) as f2:
      f1.readline()
      f2.write(header)
  ...
  def _rename_column_in_header(
      self, header: str, column_old: str, column_new: str
  ) -> str:
    ...
    idx_match = [
      i for i, item in enumerate(list_header) if re.search(pattern, item)
    ]
    if len(idx_match) != 1:
      raise SystemExit(
          "invalid count for column of %s during adjustment" % column_old
      )
...
class FALLPreprocessor(CSVPreprocessor):
  ...
  def __append_zero_to_column_if_length_below_requirement(
      self, column: str, length_required: int
  ):
    path_parent = os.path.dirname(self.PATH_CSV)
    path_dummy = os.path.sep.join([path_parent, "dummy.csv"])
    encoding = self.get_csv_encoding(self.PATH_CSV)
    ...
    for chunk in pd.read_csv(
      ...
      chunk[column] = chunk[column].apply(
          lambda x: (
            x.rjust(length_required, "0")
            if len(x) == length_required - 1
            else x
          )
      )
      df_tmp = pd.concat([df_tmp, chunk])
...
class ICDPreprocessor(CSVPreprocessor):
  ...
  def __adjust_columns_for_secondary_diagnoses(self, header: str) -> str:
    ...
    header_sub = self._rename_column_in_header(
        header_sub, "lokalisation", "sekundärlokalisation"
    )
    header_sub = self._rename_column_in_header(
        header_sub, "diagnosensicherheit", "sekundärdiagnosensicherheit"
    )
...
class CSVFileVerifier(CSVReader, ABC):
  ...
  def check_column_names_of_csv(self):
    ...
    if set_matched_columns != set_required_columns:
      raise SystemExit(
          "following columns are missing in {0}: {1}".format(
              self.CSV_NAME,
              set_required_columns.difference(set_matched_columns)
          )
      )
  ...
  def clear_invalid_column_fields_in_chunk(
      self, chunk: pd.Series, column_name: str
  ) -> pd.Series:
    """
    ...
    indeces_wrong_syntax = chunk[
      (chunk[column_name] != "") & (~chunk[column_name].str.match(pattern))
      ].index
    ...
    if len(
        indeces_empty_fields) and column_name in self.MANDATORY_COLUMN_VALUES:
...
class FALLVerifier(CSVFileVerifier):
  ...
  def is_csv_in_folder(self) -> bool:
    if not os.path.isfile(self.PATH_CSV):
      raise SystemExit(
          "fall.csv is a mandatory file and could not be found in zip"
      )
    return True
  ...
  def count_total_encounter(self) -> int:
    with open(self.PATH_CSV,
              encoding=self.get_csv_encoding(self.PATH_CSV)) as csv:
  ...
  def get_unique_ids_of_valid_encounter_with_admission_dates(self) -> dict:
    ...
    for chunk in pd.read_csv(
      ...
      for column in chunk.columns.values:
        chunk = self.clear_invalid_column_fields_in_chunk(chunk, column)
      dict_chunk = dict(
          zip(chunk["khinterneskennzeichen"], chunk["aufnahmedatum"])
      )
...
class FALLObservationFactConverter(CSVObservationFactConverter):
  ...
  def create_observation_facts_from_row(self, row_csv: pd.Series) -> list:
    facts = []
    facts.extend(
        self.__create_admission_dicts(
            row_csv["aufnahmeanlass"], row_csv["aufnahmegrund"]
        )
    )
    ...
    if row_csv["geburtsjahr"]:
      facts.extend(
          self.__create_birthyear_dicts(
              row_csv["geburtsjahr"], row_csv["aufnahmedatum"]
          )
      )
    ...
    if (
        row_csv["fallzusammenführung"] == "J"
        and row_csv["fallzusammenführungsgrund"]
    ):
      facts.append(
          self.__create_encounter_merge_dict(
              row_csv["fallzusammenführungsgrund"])
      )
    if row_csv["verweildauerintensiv"]:
      facts.append(
          self.__create_critical_care_dict(row_csv["verweildauerintensiv"])
      )
    if row_csv["entlassungsdatum"] and row_csv["entlassungsgrund"]:
      facts.append(
          self.__create_discharge_dict(
              row_csv["entlassungsdatum"], row_csv["entlassungsgrund"]
          )
      )
    ...
    if row_csv["behandlungsbeginnvorstationär"]:
      facts.append(
          self.__create_prestation_therapy_start_dict(
              row_csv["behandlungsbeginnvorstationär"],
              row_csv["behandlungstagevorstationär"],
          )
      )
    if row_csv["behandlungsendenachstationär"]:
      facts.append(
          self.__create_poststation_therapy_end_dict(
              row_csv["behandlungsendenachstationär"],
              row_csv["behandlungstagenachstationär"],
          )
      )
    return facts
  ...
  def __create_admission_dicts(cause: str, reason: str) -> list:
    ...
    return [
      {
        "concept_cd": concept_cause,
        "modifier_cd": "@",
        "valtype_cd": "@",
        "valueflag_cd": "@",
      },
      {
        "concept_cd": concept_reason,
        "modifier_cd": "@",
        "valtype_cd": "@",
        "valueflag_cd": "@",
      },
  ...
  def __create_birthyear_dicts(birthyear: str, date_admission: str) -> list:
    return [
      ...
      {
        "concept_cd": "LOINC:80904-6",
        "modifier_cd": "effectiveTime",
        "valtype_cd": "T",
        "tval_char": date_admission,
      },
    ]
  ...
  def __create_ventilation_dict(ventilation: str) -> dict:
    ...
    return {
      ...
      "modifier_cd": "@",
      "valtype_cd": "N",
      "nval_num": ventilation,
      "units_cd": "h",
    }

  @staticmethod
  def __create_prestation_therapy_start_dict(date_start: str,
      days: str) -> dict:
    """
    Date information in csv is missing hours and minutes, so dummy values are added
    """
    date_start = CSVObservationFactConverter._convert_date_to_i2b2_format(
        "".join([date_start, "0000"])
    )
    if days:
      return {
        "concept_cd": "P21:PREADM",
  ...
  def __create_poststation_therapy_end_dict(date_end: str, days: str) -> dict:
    ...
    Date information in csv is missing hours and minutes, so dummy values are added
    """
    date_end = CSVObservationFactConverter._convert_date_to_i2b2_format(
        "".join([date_end, "0000"])
    )
    if days:
      return {
        "concept_cd": "P21:POSTDIS",
        "start_date": date_end,
        "modifier_cd": "@",
        "valtype_cd": "N",
        "nval_num": days,
...
class FABObservationFactConverter(CSVObservationFactConverter):
  ...
  def create_observation_facts_from_row(self, row_csv: pd.Series) -> list:
    ...
    facts = self.__create_department_dict(
        ...
        row_csv["kennungintensivbett"],
        row_csv["fabaufnahmedatum"],
        row_csv["fabentlassungsdatum"],
    )
    return [facts]

  def __create_department_dict(
      self,
      num_instance: str,
      department: str,
      intensive: str,
    ...
    return {
      ...
      "start_date": date_start,
      "modifier_cd": "@",
      "instance_num": num_instance,
      "valtype_cd": "T",
      "tval_char": department,
      "end_date": date_end,
    }
...
class ICDObservationFactConverter(CSVObservationFactConverter):
  ...
  def create_observation_facts_from_row(self, row_csv: pd.Series) -> list:
    ...
    facts.extend(
        self.__create_icd_dicts(
            ...
            row_csv["diagnoseart"],
            row_csv["icdversion"],
            row_csv["lokalisation"],
            row_csv["diagnosensicherheit"],
        )
    )
    if row_csv["sekundärkode"]:
      ...
      facts.extend(
          self.__create_icd_sek_dicts(
              num_instance,
              row_csv["sekundärkode"],
              row_csv["icdkode"],
              row_csv["icdversion"],
              row_csv["sekundärlokalisation"],
              row_csv["sekundärdiagnosensicherheit"],
          )
      )
    return facts

  def __create_icd_dicts(
      self,
      num_instance: str,
      code: str,
      type_diag: str,
    ...
    concept = ":".join(
        ["ICD10GM", self.__convert_icd_code_to_i2b2_format(code)])
    list_facts = [
      {
        "concept_cd": concept,
        "modifier_cd": "@",
  ...
  def __create_icd_sek_dicts(
    ...
    list_facts = self.__create_icd_dicts(
        num_instance, code, "SD", version, localisation, certainty
    )
    concept_parent = ":".join(
        ["ICD10GM", self.__convert_icd_code_to_i2b2_format(code_parent)]
    )
    concept_icd = ":".join(
        ["ICD10GM", self.__convert_icd_code_to_i2b2_format(code)]
    )
    list_facts.append(
        {
          "concept_cd": concept_icd,
          "modifier_cd": "sdFrom",
          "instance_num": num_instance,
          "valtype_cd": "T",
          "tval_char": concept_parent,
        }
    )
    return list_facts
...
class OPSObservationFactConverter(CSVObservationFactConverter):
  ...
  def __create_ops_dicts(
      self, num_instance: str, code, version, localisation, date: str
  ) -> list:
