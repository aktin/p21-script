        ...
        if set_matched_columns != set_required_columns:
            raise SystemExit(
                "following columns are missing in {0}: {1}".format(
                    self.CSV_NAME, set_required_columns.difference(set_matched_columns)
                )
            )
...
class FALLObservationFactConverter(CSVObservationFactConverter):
    ...
    def create_observation_facts_from_row(self, row_csv: pd.Series) -> list:
        ...
        if (
            ...
            facts.append(
                self.__create_encounter_merge_dict(row_csv["fallzusammenführungsgrund"])
            )
...
class DatabaseEncounterMatcher:
    ...
    def get_matched_df(self, list_csv_ids: list) -> pd.DataFrame:
        ...
        df_csv = pd.DataFrame(
            list(zip(list_csv_ids, list_csv_ide)), columns=["encounter_id", "match_id"]
        )
